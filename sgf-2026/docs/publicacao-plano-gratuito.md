...
- O aplicador remoto registrou as migrations com versões geradas no momento da execução (`20260912045814` a `20260912050022`), mantendo os nomes funcionais. Não executar `supabase db push` automático contra este projeto sem reconciliar esse histórico; novas alterações devem continuar em aplicação controlada.
- Após o envio de `003b7ab` para `main`, `posto`, `oficina` e `superadmin` já servem os bundles novos. O domínio principal ainda entrega `index-BlbKofsz`/`index-CeKk2m2f`; é necessário acionar o redeploy da aplicação principal na Hostinger para concluir a distribuição do frontend.

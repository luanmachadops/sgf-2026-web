...
export const SGFInput = React.forwardRef<HTMLInputElement, SGFInputProps>(
  (
    ...
    const hasCustomPasswordToggle = Boolean(Icon && iconPosition === 'right' && onIconClick);
    const isPassword = type === 'password' && !hasCustomPasswordToggle;
      ...
      <div className={`${fullWidth ? 'w-full' : ''} min-w-0 ${className}`}>
        ...
        <div className="relative min-w-0">
          ...
          <input
            ...
            className={`${baseInputStyles} ${stateStyles} ${iconStyles} ${passwordStyles} ${inputClassName}`.trim().replace(/\s+/g, ' ')}
            {...props}
          />

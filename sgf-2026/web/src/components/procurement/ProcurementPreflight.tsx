import { AuthorizeStationContractModal } from './AuthorizeStationContractModal';
...
export function ProcurementPreflight({plan,onClose}:{plan:InstrumentBudget;onClose:()=>void}) {
  ...
  if(issuing && item && ['arla','lubricant','labor','tire_service'].includes(item.category)) return <AuthorizeStationContractModal item={item} allocationId={allocationId} initialQuantity={quantity} onClose={()=>setIssuing(false)}/>;

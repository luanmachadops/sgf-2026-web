    ...
    <div className="space-y-4">
      ...
      {query.isPending ? <p role="status">Carregando itens…</p> : query.isError ? <p role="alert" className="text-red-700">{query.error.message} <button type="button" className="underline" onClick={() => void query.refetch()}>Tentar novamente</button></p> : <>
        <div className="space-y-3">{query.data.items.map((item) => <article key={item.id} className="rounded-xl border border-slate-200 bg-white p-4">
          <p className="text-xs text-slate-500">{item.lot_reference ? `Lote ${item.lot_reference}` : 'Sem agrupamento em lote'} · {ITEM_CATEGORIES[item.category]}{item.category==='fuel' ? ` · ${item.fuel_code ? ({diesel:'Diesel',gasolina:'Gasolina',etanol:'Etanol'}[item.fuel_code]) : 'Combustível não classificado'}` : ''}</p><h3 className="mt-1 font-semibold">Item {item.reference} · {item.description}</h3>
...
function ItemEditor({ instrument, item, source, onClose, onSaved }: { instrument: ProcurementInstrument; item?: ProcurementItem; source?: ProcurementItem; onClose: () => void; onSaved: () => void }) {
  ...
  return <Modal isOpen title={item ? `Editar item ${item.reference}` : 'Novo item adjudicado'} description="Informe os dados do instrumento já formalizado. O cadastro permanece em rascunho." size="xl" onClose={() => { if (!mutation.isPending) onClose(); }}>
    <form className="space-y-4" onSubmit={submit}><fieldset disabled={mutation.isPending} className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2"><SGFSelect fullWidth label="Categoria" options={options(ITEM_CATEGORIES)} value={payload.category} disabled={Boolean(payload.origin_item_id)} onChange={(value) => update({ category: value as ItemPayload['category'], fuel_code: null })} /><SGFSelect fullWidth label="Unidade" options={options(ITEM_UNITS)} value={payload.unit} disabled={Boolean(payload.origin_item_id)} onChange={(value) => update({ unit: value as ItemPayload['unit'] })} /></div>
      {payload.category === 'fuel' && <SGFSelect fullWidth label="Combustível do item" placeholder="Classifique conforme o documento" value={payload.fuel_code ?? ''} disabled={Boolean(source)} options={[{value:'diesel',label:'Diesel'},{value:'gasolina',label:'Gasolina'},{value:'etanol',label:'Etanol'}]} onChange={value=>update({fuel_code:value as ItemPayload['fuel_code']})}/>}
    ...
    </fieldset>{mutation.isError && <p role="alert" className="text-red-700">{mutation.error.message || 'Não foi possível salvar o item. Verifique sua conexão e tente novamente.'}</p>}<div className="flex justify-end gap-2"><SGFButton type="button" variant="ghost" disabled={mutation.isPending} onClick={onClose}>Voltar</SGFButton><SGFButton type="submit" loading={mutation.isPending} disabled={!payload.partner_id || !partners.isSuccess || (payload.category === 'fuel' && !payload.fuel_code)}>Salvar item</SGFButton></div></form>
  </Modal>;
}
function PriceEditor({ instrument, item, onClose, onSaved }: { instrument: ProcurementInstrument; item: ProcurementItem; onClose: () => void; onSaved: () => void }) {
  ...
  return <Modal isOpen title={`Condição de preço · Item ${item.reference}`} description="Cada registro preserva os anteriores. Na mesma data de efeito, prevalece a revisão mais recente." size="lg" onClose={() => { if (!mutation.isPending) onClose(); }}>
    ...
    </fieldset>{mutation.isError && <p role="alert" className="text-red-700">{mutation.error.message || 'Não foi possível registrar a condição de preço. Verifique sua conexão e tente novamente.'}</p>}<div className="flex justify-end gap-2"><SGFButton type="button" variant="ghost" disabled={mutation.isPending} onClick={onClose}>Voltar</SGFButton><SGFButton type="submit" loading={mutation.isPending}>Registrar condição</SGFButton></div></form>
  </Modal>;
}

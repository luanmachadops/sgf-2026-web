...
function ImageField({ label, url, onUpload, uploading, disabled }: { label: string; url: string; onUpload: (f: File) => void; uploading: boolean; disabled: boolean }) {
    ...
    return (
        <div className="flex flex-col items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50/50 p-4">
            ...
            <input ref={ref} type="file" accept="image/*" className="hidden" disabled={disabled} onChange={(e) => { const f = e.target.files?.[0]; if (f) onUpload(f); }} />
            {!disabled && (
    ...
    return (
        <SGFCard padding="lg" className="border border-slate-200/80 shadow-sm transition-all hover:shadow-md">
            <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-500">Logo, brasão e dados que aparecem no painel.</p>
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    {!isEditing ? (
                        ...
                        <>
                            <SGFButton variant="ghost" size="sm" onClick={handleCancel} disabled={saving}>
                                Cancelar
                            </SGFButton>
                            <SGFButton size="sm" onClick={handleSave} disabled={saving} icon={saving ? Loader2 : Save}>
            ...
            <div className="grid grid-cols-3 gap-3 sm:max-w-md">
                <ImageField label="Logo" url={tenant.logoUrl} uploading={uploading === 'logo'} disabled={!isEditing} onUpload={(f) => handleUpload('logo', f)} />
                <ImageField label="Brasão" url={tenant.sealUrl} uploading={uploading === 'seal'} disabled={!isEditing} onUpload={(f) => handleUpload('seal', f)} />
                <ImageField label="Foto" url={tenant.photoUrl} uploading={uploading === 'photo'} disabled={!isEditing} onUpload={(f) => handleUpload('photo', f)} />
            </div>

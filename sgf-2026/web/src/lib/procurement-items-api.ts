...
const itemSchema = z.object({ id: z.string(), instrument_id: z.string(), reference: z.string(), lot_reference: z.string(), description: z.string(), category: z.enum(['fuel', 'arla', 'lubricant', 'parts', 'labor', 'tires', 'tire_service', 'other']), unit: z.enum(['L', 'UN', 'H', 'KM', 'KG', 'SERV']), fuel_code: z.enum(['diesel','gasolina','etanol']).nullable().default(null), quantity: z.number(), partner_kind: z.enum(['posto', 'oficina']), partner_id: z.string(), partner_name: z.string(), origin_item_id: z.string().nullable(), version: z.number(), price: priceSchema.nullable() });
export type ProcurementItem = z.infer<typeof itemSchema>;
export type ProcurementPrice = z.infer<typeof priceSchema>;
export type ItemPayload = Pick<ProcurementItem, 'instrument_id' | 'reference' | 'lot_reference' | 'description' | 'category' | 'unit' | 'quantity' | 'partner_kind' | 'partner_id' | 'origin_item_id'> & { id?: string; version?: number; reason: string; fuel_code?: ProcurementItem['fuel_code'] };

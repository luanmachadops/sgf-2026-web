...
const supplemental = process.argv.includes('--seed-station');
...
await db.exec(`select set_config('app.uid','',false);
  ...
  update public.profiles set allowed_modules=array['procurement','budgets','refuelings','stations'] where id='${admin}';
...
await setupStationWorkflow(db);
await db.exec(`select set_config('app.uid','${admin}',false); set role authenticated;`);
if (process.argv.includes('--seed-items') || (process.argv.includes('--seed-preflight') || supplemental)) {
  ...
  if ((process.argv.includes('--seed-preflight') || supplemental)) {
    for (const instrument of [ataId,contractId]) {
      await db.query('select public.save_instrument_budget($1::jsonb)',[JSON.stringify({instrument_id:instrument,fiscal_year:2026,total_limit:10000,document_reference:'Ato fictício',reason:'Teste local',allocations:[{department_id:department,category:supplemental?'arla':'fuel',spending_limit:600,appropriation:'03.01.3.3.90.30',funding_source:'001500',simam_code:''}]})]);
    }
    ...
    if(supplemental){await db.query("insert into public.procurement_station_rollouts values($1,true,'Homologação fictícia local')",[contractId]);await db.query("insert into public.station_catalog_items(tenant_id,station_id,kind,name,unit,unit_price) values($1,$2,'arla','ARLA 32','L',99)",[tenant,station]);}
    await db.exec('set role authenticated');
    let origin=null;
    for (const instrument of [ataId,contractId]) {
      const item=(await db.query('select public.save_procurement_item($1::jsonb) id',[JSON.stringify({instrument_id:instrument,reference:supplemental?'ARLA':'Diesel',description:supplemental?'ARLA 32 contratado':'Diesel para frota',category:supplemental?'arla':'fuel',fuel_code:supplemental?null:'diesel',unit:'L',quantity:100,partner_kind:'posto',partner_id:station,origin_item_id:origin,reason:'Teste local'})])).rows[0].id;
...
const rpc = {
  issue_procurement_station_operation: p => db.query('select public.issue_procurement_station_operation($1,$2::jsonb) data',[p.p_request,JSON.stringify(p.p_payload)]),
  manager_list_station_catalog: p => db.query("select coalesce(jsonb_agg(x),'[]'::jsonb) data from public.manager_list_station_catalog($1,false) x",[p.p_station_id]),
...
const server = await createServer({
  ...
  plugins: [{
    ...
    resolveId(source, importer) {
      ...
      if ((source === './supabase' || /\/supabase(?:\.ts)?$/.test(source)) && (importer?.endsWith('procurement-registry-api.ts') || importer?.endsWith('procurement-items-api.ts') || importer?.endsWith('instrument-budget-api.ts') || importer?.endsWith('station-operations-api.ts'))) return '\0preview-rpc';
    ...
    load(source) {
      ...
      if (source === '\0preview-auth') return `export const useAuth=()=>({user:{id:'${admin}',tenantId:'${tenant}',accountRole:'admin',allowedModules:['procurement','budgets','refuelings','stations']}});`;
